...
def preprocess_text(
    sp, text: str, user_data=None
):  ### takes text as a string (not list) and return list
    text = text.lower()
    text = [
        word.lemma_
        for word in sp(text)
        if word.is_alpha and (not word.is_stop) and len(word) > 3
    ]
...
def create_corpus(data, language):
    ...
    for i, doc in enumerate(raw_articles):
        if i % 500 == 1:
            print(
                "Runtime: %.2f seconds" % (time.time() - start),
                "|| Completed: %s of %s" % (i, num_articles),
            )
...
for k1 in range(1, k1_max):
    for k2 in range(k1, k2_max):
        ...
        score = tp.coherence.Coherence(
            model, coherence="c_v"
        ).get_score()  # try with 'u_mass' instead of 'c_v'
        params.append((k1, k2))
        scores.append(score)
        print(
            "Runtime: %.2f seconds" % (time.time() - start),
            "|| Number of Searches: %s out of  %s" % (num_iter, max_iter),
        )
...
print(
    "(BEST MODEL) k1=%s k2=%s coherence=%.2f"
    % (params[best_params][0], params[best_params][1], scores[best_params])
)

data = pd.DataFrame(
    data={"k1": list(zip(*params))[0], "k2": list(zip(*params))[1], "score": scores}
)
...
model = tp.PAModel(
    tw=tp.TermWeight.IDF, min_cf=5, k1=best_k1, k2=best_k2, corpus=corpus, seed=0
)
model.burn_in = 100
